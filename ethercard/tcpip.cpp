...
//RSL {
    ...
    if ((dip[0] & 0xF0) == 0xE0 || *((unsigned long*)dip) == 0xFFFFFFFF ||
    		*((unsigned long*)dip) == *((unsigned long*)broadcastip))
//RSL }
...
//RSL {
void EtherCard::udpPrepareBroadcast(uint16_t sport, uint16_t dport) {
  *(unsigned long*)hisip = *(unsigned long*)broadcastip;
  udpPrepare (sport, hisip, dport);
}

void EtherCard::sendUdpBroadcast(const char *data, uint8_t datalen, uint16_t sport,
                         uint16_t dport) {
  *(unsigned long*)hisip = *(unsigned long*)broadcastip;
  sendUdp(data, datalen, sport, hisip, dport);
}
//RSL }
